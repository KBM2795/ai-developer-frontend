...
const debouncedSend = debounce((message, user, setMessage, setMessages, messages, setIsSending, project, setIsAiTyping) => {
...
export const Project = () => {
  ...
  const [isAiTyping, setIsAiTyping] = useState(false); // Add this state
  ...
  const [newFileName, setNewFileName] = useState(''); // Add this new state near other state declarations
  ...
  function send() {
    debouncedSend(message, user, setMessage, setMessages, messages, setIsSending, project, setIsAiTyping);
  }
...
useEffect(() => {
  ...
  const themeClasses = {
    ...
    sidebarBg: darkMode ? 'bg-[#121212]' : 'bg-white',
    headerBg: darkMode ? 'bg-black border-gray-800' : 'bg-white border-gray-200',
    cardBg: darkMode ? 'bg-[#121212]' : 'bg-white',
    inputBg: darkMode ? 'bg-[#1a1a1a] text-white border-gray-800' : 'bg-gray-200 text-gray-900 border-gray-300',
    buttonPrimary: darkMode ? 'bg-gradient-to-r from-[#2a9d8f] to-[#64dfdf] hover:opacity-90' : 'bg-blue-500 hover:bg-blue-600',
    messageReceived: darkMode ? 'bg-[#1a1a1a]' : 'bg-gray-100',
    messageSent: darkMode ? 'bg-[#2a9d8f]' : 'bg-blue-500',
    modalBg: darkMode ? 'bg-[#121212] text-white' : 'bg-white text-gray-900',
    border: darkMode ? 'border-gray-800' : 'border-gray-200',
    hoverBg: darkMode ? 'hover:bg-[#1a1a1a]' : 'hover:bg-gray-100',
    activeTab: darkMode ? 'bg-[#1a1a1a] text-[#64dfdf]' : 'bg-gray-200 text-gray-900',
    inactiveTab: darkMode ? 'text-gray-500 hover:text-[#64dfdf]' : 'text-gray-500 hover:text-gray-900'
  };
   ...
   const resize = useCallback(
    (mouseMoveEvent) => {
      if (isResizing) {
        ...
        if (newWidth >= 300 && newWidth <= 800) { // Min and max width limits
  ...
  return (
    ...
    >
      {/* Remove glowing background elements */}
      <motion.section 
        style={{ width: sidebarWidth ,minWidth:'350px',maxWidth:'800px' }}
        className={`left flex flex-col h-full backdrop-blur-md bg-black/30 border-r ${themeClasses.border} relative z-10`}
      ...
      >
        {/* Add resize handle */}
        <div
          className={`absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-[#121212] ${isResizing ? 'bg-blue-500' : ''}`}
        ...
        <div className="flex-grow overflow-hidden">
          {/* Chat Tab */}
                ...
                <div className="h-full flex flex-col">
                  ...
                  {isAiTyping && (
                    <div className="message max-w-64 flex items-center gap-2 p-3 rounded-lg bg-[#1e1e1e] text-[#d4d4d4] mr-auto" style={{ maxWidth: '90%' }}>
                      <small className="opacity-75 text-xs">AI</small>
            ...
            <div className="flex flex-col h-full">
              <div className="p-3 border-b flex">
                <input
                  ...
                  className={`flex-grow p-2 rounded-l-md ${themeClasses.inputBg} border-r-0`}
                />
                <button
                  onClick={createNewFile}
                  className={`px-4 py-2 rounded-r-md ${themeClasses.buttonPrimary} text-white`}
      ...
      >
        <motion.header 
          className={`px-6 py-4 border-b backdrop-blur-md bg-black/30 ${themeClasses.border}`}
          initial={{ y: -10 }}
          animate={{ y: 0 }}
          transition={{ duration: 0.2 }}
        >
      ...
      <AnimatePresence>
        {isModalOpen && (
          ...
          >
            <motion.div
              className="bg-black/50 backdrop-blur-md p-6 rounded-lg border border-gray-800/50 w-96 max-w-full"
